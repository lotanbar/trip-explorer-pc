...
- Only notable OSM POIs are fetched: named and carrying a `wikidata`, `wikipedia` or `heritage` tag (an `(if: …)` filter on every query clause). Everything else swamped the map.
